function main()
{
    ...
    for ( var i = 0; i < 256; i++ )
    {
        var S = scale_convert(i);
        var R = 1;
        // var R = Math.max( Math.cos( ( (S - 0.1) / 0.7 - 1 ) * Math.PI ), 0.0 );
        // var G = Math.max( Math.cos( ( (S - 0.1) / 0.7 - 0.5 ) * Math.PI ), 0.0 );
        // var B = Math.max( Math.cos( ((S - 0.1) / 0.7) *  Math.PI ), 0.0 );
        var G = Math.max( Math.cos( ((S - 0.1) / 0.7) *  Math.PI ), 0.0 );
        var B = Math.max( Math.cos( ((S - 0.1) / 0.7) *  Math.PI ), 0.0 );
    ...
    {
        var id = faces[i];
        var S0 = scale_convert2(scalars[ id[0] ]);
        var S1 = scale_convert2(scalars[ id[1] ]);
        var S2 = scale_convert2(scalars[ id[2] ]);
    ...
    function scale_convert(num)
    {
        return (num * 0.7) / 255.0 + 0.1;
    }

    function scale_convert2(num2)
    {
        return Math.round(((num2 - 0.1) * 255.0) / 0.7);
    }
}
